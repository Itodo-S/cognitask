...
export async function todoRoutes(app: FastifyInstance) {
  ...
  // Declared before /:id so "stats" is not swallowed by the id param.
  ...
  app.post("/api/todos/batch", async (request, reply) => {
    ...
    for (const todo of todos) wsGateway.broadcast("todo:created", todo);
  ...
  app.delete<{ Params: { id: string } }>("/api/todos/:id", async (request, reply) => {
    ...
    wsGateway.broadcast("todo:archived", { id: request.params.id });
    return reply.send(success(null, "Todo archived"));
  });

  app.post<{ Params: { id: string } }>("/api/todos/:id/restore", async (request, reply) => {
    const todo = await todoService.restore(request.params.id);
    if (!todo) return reply.code(404).send(error("Archived todo not found"));
    wsGateway.broadcast("todo:updated", todo);
    return reply.send(success(todo, "Todo restored"));
  });
  ...
  app.post<{ Params: { id: string } }>("/api/todos/:id/subtasks", async (request, reply) => {
    ...
    wsGateway.broadcast("todo:created", subtask);
